...
void attention_forward_cudnn(floatX* out,  // output: (B, T, NH, HS)
    ...
    if (graph->get_workspace_size() > cudnn_workspace_size) {
        if (cudnn_workspace_size > 0) {
            cuda_free(cudnn_workspace);
        }
        cudnn_workspace_size = graph->get_workspace_size();
        cuda_malloc(&cudnn_workspace, cudnn_workspace_size);
    }
...
void create_cudnn_handle() {
    checkCudnnErr(cudnnCreate(&cudnn_handle));
}

void destroy_cudnn_handle() {
    if (cudnn_workspace != NULL) { cuda_free(cudnn_workspace); }
